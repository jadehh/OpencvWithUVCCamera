...
public class Mp4MediaMuxer {
    ...
    public Mp4MediaMuxer(String path, long durationMillis, boolean isVoiceClose) {
        ...
        this.mFilePath = path;
        if(durationMillis != 0) {
            filePath = path + DASH + index +VIDEO_SUFFIX;
        }else{
            filePath = path + VIDEO_SUFFIX;
        }
        Object mux = null;
        try {
    ...
    public synchronized void addTrack(MediaFormat format, boolean isVideo) {
        ...
        if (VERBOSE){
            Log.e(TAG,"Build.VERSION.SDK_INT"+String.valueOf(Build.VERSION.SDK_INT) + " Build.VERSION_CODES.JELLY_BEAN_MR2"+String.valueOf( Build.VERSION_CODES.JELLY_BEAN_MR2));
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            int track = mMuxer.addTrack(format);
            if (VERBOSE)
                Log.e(TAG,String.format("addTrack %s result %d", isVideo ? "video" : "audio", track));
            ...
            if (isVideo) {
                ...
                if (isVoiceClose || mAudioTrackIndex != -1) {
                    if (VERBOSE)
                        Log.e(TAG,"both audio and video added,and muxer is started");
                    mMuxer.start();
...
//                Log.i(TAG, "BUFFER_FLAG_END_OF_STREAM received");
        ...
        Log.e(TAG,"delta time " + (System.currentTimeMillis() - mBeginMillis));
...
//                    Log.i(TAG, String.format("record file reach expiration.create new file:" + index));
                ...
                try {
                    mMuxer = new MediaMuxer(mFilePath + "-" + ++index + VIDEO_SUFFIX, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
    ...
    public synchronized void release() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            if (mMuxer != null) {
                ...
                if (mVideoTrackIndex  != -1) {
                    if (VERBOSE)
                        Log.e(TAG,String.format("muxer is started. now it will be stoped."));
                    try {
                        mMuxer.stop();
                        mMuxer.release();
                    ...
                    if (System.currentTimeMillis() - mBeginMillis <= 1500){
                        new File(mFilePath + "-" + index + VIDEO_SUFFIX).delete();
                    }
                    mAudioTrackIndex = mVideoTrackIndex = -1;
                }else{
                    if (VERBOSE)
                        Log.e(TAG,String.format("muxer is failed to be stoped."));
                }
            }
        }
    }
