...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
	...
	public synchronized void setAutoFocus(final boolean autoFocus) {
		if (mNativePtr != 0) {
			...
			Log.e(TAG,"set Auto Focus"+String.valueOf(result));
		}
	}
...
//================================================================================
	...
	public synchronized void setExposureMode(final int mode){
		if (mNativePtr != 0) {
			...
			if (DEBUG){
				if (result == 0){
					Log.e(TAG,"setExposureMode Success");
				}
				else{
					Log.e(TAG,"setExposureMode Failed");
				}
			}
	...
	public synchronized void setExposureValue(int value){
		...
		if (DEBUG){
			if (result == 0){
				Log.e(TAG,"setExposureValue Success");
			}
			else{
				Log.e(TAG,"setExposureValue Failed");
			}
		}
	}
