...
public class MtionDet2 {
    ...
    public List<Integer> detect(Mat frame, int idx){
        ...
        System.out.println("index = "+String.valueOf(idx)+",num_pixel = "+String.valueOf(num_pixel));
//        System.out.println("a = "+String.valueOf(frame.get(100,100)[0])+"b = "+String.valueOf(frame.get(100,100)[1])+"c = "+String.valueOf(frame.get(100,100)[2]));
        ...
        if(m_num_cache.get(2) == 0 && m_num_cache.get(3) == 0 && m_num_cache.get(4)==0){
            if (m_num_cache.get(5)> 0 && m_num_cache.get(6)>0 && m_num_cache.get(7)>0&&(m_num_cache.get(5)+m_num_cache.get(6)+m_num_cache.get(7))>16){
        ...
        if(m_num_cache.get(2) > 0 && m_num_cache.get(3) > 0 && m_num_cache.get(4) >0 && (m_num_cache.get(2) + m_num_cache.get(3) + m_num_cache.get(4)) > 16) {
            if (m_num_cache.get(5) == 0 && m_num_cache.get(6) == 0 && m_num_cache.get(7) == 0) {
                for (int i=2;i<9;i++){
                    int frame_id = frame_id_cache.get(i);
                    if (!result_id.contains(frame_id)){
                        result_id.add(frame_id);
                    }
                }
            }
